...
static int
parse_pmd_sleep_list(const char *max_sleep_list, struct pmd_val **pmd_vals)
{
    char *list, *copy, *key, *value;
    int num_vals = 0;
    ...
    while (ofputil_parse_key_value(&list, &key, &value)) {
        uint64_t temp, core, pmd_max_sleep;
        int i;

        if (str_to_u64(key, &temp)) {
            break;
        }

        if (str_to_u64(value, &pmd_max_sleep)) {
            /* No value specified, this is the global default. */
            core = UINT64_MAX;
            pmd_max_sleep = temp;
        } else {
            /* Value specified, store the core number. */
            core = temp;
        }

        /* Detect duplicate max sleep values for default or a specific core. */
        for (i = 0 ; i < num_vals; i++) {
            if ((*pmd_vals)[i].core_id == core) {
                break;
            }
        }
        if (i == num_vals) {
            /* This is not a duplicate, add a new entry.*/
            *pmd_vals = xrealloc(*pmd_vals,
                                 (num_vals + 1) * sizeof **pmd_vals);
            num_vals++;
        }

        pmd_max_sleep = MIN(PMD_RCU_QUIESCE_INTERVAL, pmd_max_sleep);

        (*pmd_vals)[i].core_id = core;
        (*pmd_vals)[i].max_sleep = pmd_max_sleep;
    }
...
static void log_pmd_sleep(int core_id, int numa_id, uint64_t pmd_max_sleep,
                          int default_max_sleep)
{
...
{
    struct pmd_val *pmd_vals = NULL;
    uint64_t max_sleep = UINT64_MAX;
    int num_vals;

    num_vals = parse_pmd_sleep_list(dp->max_sleep_list, &pmd_vals);

    /* Check if the user has set a default value. */
    for (int i = 0 ; i < num_vals; i++) {
        if (pmd_vals[i].core_id == pmd->core_id) {
            max_sleep = pmd_vals[i].max_sleep;
            break;
        }
    }
                       ...
                       bool always_log)
{
    ...
    if (nullable_string_is_equal(max_sleep_list,
                                 dp->max_sleep_list) && !always_log) {
        return;
    }
    ...
    num_vals = parse_pmd_sleep_list(max_sleep_list, &pmd_vals);

    default_max_sleep = 0;
    /* Check if the user has set a default pmd value. */
    for (int i = 0 ; i < num_vals; i++) {
        if (pmd_vals[i].core_id == UINT64_MAX) {
            default_max_sleep = pmd_vals[i].max_sleep;
            break;
        }
    }

    /* Set default max sleep if changed. */
    ...
    CMAP_FOR_EACH (pmd, node, &dp->poll_threads) {
        ...
        for (int i = 0 ;  i < num_vals; i++) {
            if (pmd_vals[i].core_id == pmd->core_id) {
                new_max_sleep = pmd_vals[i].max_sleep;
                break;
            }
        }
